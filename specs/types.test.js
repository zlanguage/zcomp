...
context("Function Typing Tests", () => {
  describe("Tokenization (tokenize.js)", () => {
    ...
    it("should have returned a function that returns a proper token.", () => {
      expect(testTok).to.have.any.keys([
        "id",
        "lineNumber",
        "columnNumber",
        "columnWidth"
  ...
  describe("Parsing (parse.js)", () => {
    ...
    it("should return an array of objects.", () => {
      expect(testAst).to.satisfy(ast =>
        ast.every(node => typeof node === "object")
      )
    })
  })
  describe("Code Generation (gen.js)", () => {
    ...
    it("should return a string that imports the Z Standard Library.", () => {
      expect(testGened).to.satisfy(testGened =>
        testGened.includes(`const $Z = require("@zlanguage/zstdlib")`)
      )
    })
    it("should return a string that sets strict mode to true.", () => {
      expect(testGened).to.satisfy(testGened =>
        testGened.startsWith(`"use strict"`)
      )
    })
  })
})
