    ...
    if (tok !== undefined) {
        switch (tok.id) {
            ...
            default:
                ...
                if (tok.alphanumeric || (prevTok.id === "[" && tok.id === ":") || tok.id === ")" || tok.id === "]" || tok.id === "@") {
...
function statement() {
    ...
    if (tok && tok.id === "(keyword)" && !exprKeywords.includes(tok.string)) {
        ...
        if (typeIn(res, "assignment") || typeIn(res, "invocation") || typeIn(res, "get") || typeIn(res, "match") || res && res.species === "Destructuring[Array]") { // If res is a valid standalone expression, return it.
            return res;
        } else {
