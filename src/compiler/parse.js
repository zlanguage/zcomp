    ...
    switch (nextTok.id) {
      ...
      case "[":
        ...
        if (typeIn(wunth, "assignment") || wunth === ":") {
          ...
          } else if (wunth.type === "assignment") {
            // Handle dual slice [start:end]
            type = "refinement";
            const start = wunth.zeroth;
          ...
          if (wunth === ":") {
            ...
            if (nextTok.id === "]") {
              ...
              if (tok.number === undefined) {
                return error(
                  "For now, end slices (slices in the format [:_]) can only accept raw numeric literals."
                );
              }
  ...
  if (tok && tok.id === "(keyword)" && !exprKeywords.includes(tok.string)) {
    ...
    if (
      ...
      if (res !== undefined) {
        ...
        return error(
          "Invalid expression, expression must be an assignment or invocation if it does not involve a keyword."
        );
      }
    }
  }
