...
function tokenize(source, comment = false) {
  ...
  // Configure source
  const lines = Array.isArray(source) ? source : source.split(newLineRegExp);
  let lineNumber = 0;
  ...
  return function tokenGenerator() {
    ...
    if (captives[5]) {
      let res = captives[5];
      if (!dotLast) {
        // If a . wasn't last (the identifier isn't part of a refinement/property access)
        // Mangle it
      ...
      return {
        ...
        number: Number(
          captives[6]
            ...
            .replace(/[a-z]$/, "")
            .replace(/_/g, "")
        ), // Make the number JS-Compatable for parsing
        string: captives[6],
        lineNumber,
        columnNumber,
        columnTo,
