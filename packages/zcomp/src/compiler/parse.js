...
function formatCurrLine() {
  ...
  return `${lastTok.lineNumber + 1}:${lastTok.columnNumber}:${
    lastTok.columnTo
  }`;
}
...
function arrayToObj(arr) {
  ...
  const obj = arr
    ...
    .map(({ zeroth, wunth }) => [zeroth, wunth])
    .map(([zeroth, wunth]) => [arrayToObj(zeroth), arrayToObj(wunth)]);
  obj.species = "Object";
  return obj;
}
...
function isExprAhead() {
  return (
    ...
    ["(", "[", ".", "..", ":"].includes(nextTok.id) &&
    (["(", "["].includes(nextTok.id)
      ? tok.lineNumber === nextTok.lineNumber
      : true)
...
function findImplicits(ast) {
  if (
    (typeof ast !== "object" && typeof ast !== "string") ||
    ast === null ||
    ast.type === "match"
  ) {
    return [];
...
function wrapReturn(returnType, block) {
  ...
  if (Array.isArray(block)) {
    block = block.map((statement, index) =>
      convertReturns(returnType, statement, index)
    );
  }
  if (block.zeroth) {
    if (Array.isArray(block.zeroth)) {
      block.zeroth = block.zeroth.map((statement, index) =>
        convertReturns(returnType, statement, index)
      );
    } else {
  ...
  if (block.wunth) {
    if (Array.isArray(block.wunth)) {
      block.wunth = block.wunth.map((statement, index) =>
        convertReturns(returnType, statement, index)
      );
    } else {
  ...
  if (block.twoth) {
    if (Array.isArray(block.twoth)) {
      block.twoth = block.twoth.map((statement, index) =>
        convertReturns(returnType, statement, index)
      );
    } else {
...
function warnBadOp(str) {
  ...
  syms.forEach((sym) => {
    if (str.id.includes(sym)) {
      warn(
        `You may have intended to use ${demangle(
          sym
        )} as an operator in this context. To do so, separate the operator and it's operands. ${
          str.source
        } is interpreted as an alphanumeric identifier without spaces.`
      );
    }
  });
}
...
function expr({ infix = true } = {}) {
          ...
          case "func": {
            ...
            if (nextTok && nextTok.id === "(") {
              ...
              if (nextTok && nextTok.id === "{") {
                ...
                tok.id === ")" &&
                nextTok &&
                nextTok.alphanumeric &&
                nextTok.id.endsWith("$exclam")
              ) {
                // Or handle a return type
                advance(")");
                ...
                ]; // Wrap the function body's return statements with type assertions.
              } else {
  ...
  if (nextTok !== undefined && type !== "spread") {
    ...
    switch (nextTok.id) {
      case ".":
        ...
        if (!tok) {
          return error(
            `Infix operation refinement requires right-hand side for property access. Only left-hand provided.`
          );
        }
        ...
        if (tok.number !== undefined && typeof zeroth === "number") {
          ...
          if (isExprAhead()) {
            ...
            wunth = temp.wunth;
            twoth = temp.twoth;
            ({ type, zeroth, wunth, twoth } = mkChain(
              type,
              zeroth,
              wunth,
              twoth
          ...
          if (!tok.alphanumeric && tok.id !== "(keyword)") {
            return error(
              `Refinement expects valid identifier but instead got ${tok.id}.`
            );
          }
          ...
          if (isExprAhead()) {
            ...
            ({ type, zeroth, wunth, twoth } = mkChain(
              ...
              wunth,
              twoth
            )); // Support operators with refinments.
          }
        }
        break;
      case "..":
        ...
        if (isExprAhead()) {
          ...
          ({ type, zeroth, wunth, twoth } = mkChain(
            ...
            wunth,
            twoth
          )); // Support operators for optional chaining.
        }
        break;
      case "...":
        // Parse range
      ...
      case "(":
        ...
        if (
          wunth.length !== 0 &&
          wunth.every((param) => param && param.type === "assignment")
        ) {
          wunth = [
            wunth.map(({ zeroth, wunth }) => [`"` + zeroth + `"`, wunth]),
          ];
        ...
        if (isExprAhead()) {
          ...
          ({ type, zeroth, wunth, twoth } = mkChain(
            ...
            wunth,
            twoth
          )); // Support operators with invocations.
        }
        break;
      case "[":
        // Parse subscript
  ...
  if (
    nextTok &&
    nextTok.alphanumeric &&
    (validStartLineOps.includes(nextTok.id)
      ? true
      : nextTok.lineNumber === tok.lineNumber) &&
    !nextTok.id.endsWith("$exclam") &&
    nextTok.id !== "$eq$gt" &&
    (isValidOp(nextTok.id) || nextTok.id === "pow") &&
    infix
  ) {
...
parseStatement.if = (extraAdv) => {
  ...
  if (tok.id === "(") {
    warn(
      "If conditions in Z don't have parentheses around them. This could lead to a misinterpretation of the entire if statement. Try `if cond {` rather than `if (cond) {`."
    );
  }
...
parseStatement.return = () => {
  ...
  if (returnStatement.zeroth.id === "(error)") {
    warn(
      "You may have a bare return statement in your code. These are not allowed in Z. Use `return undefined` instead."
    );
  }
...
parseStatement.enum = () => {
  ...
  if (nextTok.id === "(") {
    ...
    if (
      !res.wunth.every(
        (part) => typeof part === "string" || part.type === "invocation"
      )
    ) {
      return error(
        "Only parenless constructors and normal constructors are allowed in enum declarations."
      );
    }
  }
  ...
  if (
    !res.wunth
      .filter((part) => part.type === "invocation")
      .every((invok) =>
        invok.wunth.every(
          (part) => Array.isArray(part) || typeof part === "string"
        )
      )
  ) {
    return error(
      `Error in enum constructor parameter list. Enum name: ${res.zeroth}.`
    );
  }
  ...
  if (nextTok && nextTok.id === "(keyword)" && nextTok.string === "derives") {
    ...
    res.staticDerives = derives
      .filter((derive) => derive.type === "static")
      .map((derive) => derive.zeroth);
  }
...
parseStatement.macro = function () {
    ...
    gen([
      {
        ...
        wunth: [
          {
            type: "function",
            zeroth: params.map((param) =>
              param.paramName ? param.paramName : param.name
            ),
            wunth: block(),
          },
        ],
  ...
  if (Object.keys(macros).includes(macroName)) {
    ...
    if (oldMacro.divergencePoint === undefined) {
      ...
      macros[macroName].options.push(
        params[macros[macroName].divergencePoint].name
      );
...
function statement() {
  ...
  if (tok && tok.id === "(keyword)" && !exprKeywords.includes(tok.string)) {
    ...
    // Otherwise, it's some kind of expressions
    let res = expr();
    ...
    if (
      typeIn(res, "assignment") ||
      typeIn(res, "invocation") ||
      typeIn(res, "get") ||
      typeIn(res, "match") ||
      typeIn(res, "function") ||
      (res && res.species === "Destructuring[Array]")
    ) {
      // If res is a valid standalone expression, return it.
      return res;
    } else {
      if (res !== undefined && !isMacro) {
        // Otherwise, report an error.
        return error(
          `Invalid expression ${res}, expression must be an assignment or invocation if it does not involve a keyword.`
        );
      }
      return res;
    }
  ...
  switch (typeof node) {
    ...
    case "object":
      ...
      if (Array.isArray(node.zeroth)) {
        node.zeroth = node.zeroth.map((node) =>
          insertParams(node, params, cache)
        );
      }
      if (Array.isArray(node.wunth)) {
        node.wunth = node.wunth.map((node) =>
          insertParams(node, params, cache)
        );
      }
      if (Array.isArray(node.twoth)) {
        node.twoth = node.twoth.map((node) =>
          insertParams(node, params, cache)
        );
      }
      break;
...
function paramMacro() {
  ...
  macroParams.every(function handle(param, p = params) {
    ...
    if (p === macros[macroName].divergencePoint) {
      ...
      if (correctPath === -1) {
        err = error(
          `Macro "${macroName}" is only overloaded for the following identifiers: ${ops
            .map((op) => `"${op}"`)
            .join(", ")}. There is no overload available for "${tok.id}".`
        );
        return false;
      }
      params[name] = name.startsWith('"') ? name : '"' + name + '"';
      macros[macroName].paths[correctPath].params
        .slice(macros[macroName].divergencePoint + 1)
        .forEach((param, p) => {
  ...
  if (correctPath !== undefined) {
    macroString = macros[macroName].paths[correctPath].macro(
...
function statements() {
  ...
  while (true) {
    ...
    if (
      nextStatement.type === undefined &&
      nextTok === undefined &&
      nextStatement.id !== "(error)"
    ) {
      if (!isMacro) {
        break;
      }
...
function resolveOpMacros(ast) {
  if (Array.isArray(ast)) {
    ast.forEach((statement, index) => {
      if (
        statement.type === "invocation" &&
        Object.keys(macros).includes(statement.zeroth)
      ) {
      ...
      if (
        statement.zeroth &&
        statement.zeroth.type === "invocation" &&
        Object.keys(macros).includes(statement.zeroth.zeroth)
      ) {
        statement.zeroth = resolveOpMacros([statement.zeroth])[0];
      }
      if (
        statement.wunth &&
        statement.wunth.type === "invocation" &&
        Object.keys(macros).includes(statement.wunth.zeroth)
      ) {
        statement.wunth = resolveOpMacros([statement.wunth])[0];
      }
      if (
        statement.twoth &&
        statement.twoth.type === "invocation" &&
        Object.keys(macros).includes(statement.twoth.zeroth)
      ) {
...
function parse(tokGen, debug = true) {
  ...
  if (warnings.length > 0 && debug) {
    console.log(
      `${warnings.length} warning${warnings.length === 1 ? "" : "s"} generated:`
    );
  ...
  if (!findAndThrow(statementz)) {
    ...
    if (isGoroutine(statementz)) {
      if (statementz.some((statement) => statement.type === "export")) {
        throw error(
          "Export and top-level get do not work well together. Until top-level await is supported, another solution is needed. Try exporting a promise instead."
        ).zeroth;
      }
      return [
        new AstNode({
  ...
  if (debug) {
    console.log(
      `${errors.length} error${errors.length === 1 ? "" : "s"} generated:`
    );
    errors.forEach((error) => {
      console.log(error.zeroth);
    });
  } else {
    // Otherwise, just throw the first one.
